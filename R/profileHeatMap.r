...
#' @param criteria Vector of criteria values to be used as contours on heatmap plot.
#' @importFrom akima interp
#' @importFrom lubridate origin

#' @export
profileHeatMap=function(data, parameter, depth="Depth_m", param_units, depth_units="m", date=as.Date(data$ActivityStartDate), show_dates=TRUE, min_date=min(data[,date], na.rm=T), max_date=max(data[,date], na.rm=T), param_lab="pH", criteria){
