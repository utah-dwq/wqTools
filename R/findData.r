	...
	server=function(input, output, session){
		...
		observeEvent(input$query_sites, {
			...
			if("USGS gauges" %in%  input$data_types){
				...
				try(reactive_objects$gauges <- dataRetrieval::readNWISdata(bBox=c(round(map_box[[4]],7), round(map_box[[3]],7), round(map_box[[2]],7), round(map_box[[1]],7)), service="site", hasDataTypeCd="iv"))
			}
		...
		observe({
			...
			if(!is.null(reactive_objects$wqp_sites) & "Monitoring locations" %in%  input$data_types){
			...
			if(!is.null(reactive_objects$gauges) & "USGS gauges" %in%  input$data_types){
			...
			if(!is.null(reactive_objects$permits) & "Permits" %in%  input$data_types){
